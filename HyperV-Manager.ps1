<#
    ...
    .CHANGELOG
        ...
        B.E.T.A. 1.9.6 - MinersWin - 18.04.2019 - Möglichkeit zur Speicherung der Log Datei
    ...
    .PROBLEMBEHANDLING
      Sollte es zu so einem Fehler kommen, "Die Datei .\HyperV-Manager.ps1 kann nicht geladen werden, da die Ausführung von Skripts auf diesem System deaktiviert ist." lässt sich dieses durch das Ausführn des Folgenden Befehls beheben:              Set-ExecutionPolicy Unrestricted -Scope Process
#>
...
$Button13.Text = [System.String]'Aktualisieren'
...
$Button8.Add_Click({ Start-VM })
$Button9.Add_Click({ Stop-VM })
...
. InitializeComponent
Write-Output "" >> $env:APPDATA\..\Local\temp\log.txt
rm $env:APPDATA\..\Local\temp\log.txt
Write-Output "" >> $env:APPDATA\..\Local\temp\log.txt
Write-Output "" >> $env:APPDATA\..\Local\temp\log.txt
Write-Output "                   _______  _______  _______                 " >> $env:APPDATA\..\Local\temp\log.txt
Write-Output "|\     /||\     /||  ____ ||  ____ \|  ____ |       |\     /|" >> $env:APPDATA\..\Local\temp\log.txt
Write-Output "| |   | || \   / || |    ||| |    \/| |    ||       | |   | |" >> $env:APPDATA\..\Local\temp\log.txt
Write-Output "| |___| | \ \_/ / | |____||| |__    | |____|| _____ | |   | |" >> $env:APPDATA\..\Local\temp\log.txt
Write-Output "|  ___  |  \   /  |  _____||  __|   |    ___||_____|| |   | |" >> $env:APPDATA\..\Local\temp\log.txt
Write-Output "| |   | |   | |   | |      | |      | |\ \           \ \_/ / " >> $env:APPDATA\..\Local\temp\log.txt
Write-Output "| |   | |   | |   | |      | |____/\| | \ \_          \   /   " >> $env:APPDATA\..\Local\temp\log.txt
Write-Output "|/     \|   |_|   |/       |_______/|/   \__|          \_/   " >> $env:APPDATA\..\Local\temp\log.txt
Write-Output "                                                             " >> $env:APPDATA\..\Local\temp\log.txt
Write-Output " _________  _______  _        _______  _______  _______  _______ " >> $env:APPDATA\..\Local\temp\log.txt
Write-Output "|  _   _  ||  ___  || \    /||  ___  ||  ____ \|  ____ \|  ____ |" >> $env:APPDATA\..\Local\temp\log.txt
Write-Output "| | | | | || |   | ||  \  | || |   | || |    \/| |    \/| |    ||" >> $env:APPDATA\..\Local\temp\log.txt
Write-Output "| | | | | || |___| ||   \ | || |___| || |      | |__    | |____||" >> $env:APPDATA\..\Local\temp\log.txt
Write-Output "| | |_| | ||  ___  || |\ \| ||  ___  || | ____ |  __|   |     __|" >> $env:APPDATA\..\Local\temp\log.txt
Write-Output "| |     | || |   | || | \   || |   | || | \_  || |      | |\  |  " >> $env:APPDATA\..\Local\temp\log.txt
Write-Output "| |     | || |   | || |  \  || |   | || |___| || |____/\| | \ \__" >> $env:APPDATA\..\Local\temp\log.txt
Write-Output "|/       \||/     \||/    |_||/     \||_______||_______/|/   \__/" >> $env:APPDATA\..\Local\temp\log.txt
...
function Test-Version{
    ...
    $Update.Content
    if ($Update.Content -gt "196"){
        Write-Host "Update Erforderlich: https://pastebin.com/zSqQXhus"
    [System.Windows.Forms.MessageBox]::Show("Update ist Erforderlich, Bitte Update das Skript um die neusten Funktionen und Support zu erhalten. Alte Versionen werden nicht mehr Supportet. Aktuellste Version: https://pastebin.com/zSqQXhus","MinersWin Hyper-V Manager V.2",1)
    } elseif ($Update.Content -lt 196) {
        Write-Host "Fehler"
...
function Test-IsAdmin {
    try {
        $Datum = Get-Date
        Write-Output "$Datum Suche nach Adminrechten" >> $env:APPDATA\..\Local\temp\log.txt
        $identity = [Security.Principal.WindowsIdentity]::GetCurrent()
        $principal = New-Object Security.Principal.WindowsPrincipal -ArgumentList $identity
        return $principal.IsInRole( [Security.Principal.WindowsBuiltInRole]::Administrator )
    } catch {
        Write-Output "$Datum Fehler beim Überprüfen der Adminrechten" >> $env:APPDATA\..\Local\temp\log.txt
        throw "Fehler beim eststellen der Benutzerrechte. Error: '{0}'." -f $_
    }
  }
...
if (-not(Test-IsAdmin)) {
    $Datum = Get-Date
    Write-Output "$Datum Programm wurde ohne Adminrechte gestartet" >> $env:APPDATA\..\Local\temp\log.txt
    ...
    $Datum = Get-Date
    Write-Output "$Datum Programm wurde mit Adminrechten gestartet" >> $env:APPDATA\..\Local\temp\log.txt
}
...
function Load-ComboBox-VMs{ #VM Auswahl
            $Datum = Get-Date
            Write-Output "[Lade VMs]" >> $env:APPDATA\..\Local\temp\log.txt
    $ComboBox1.Items.Clear()
            $Datum = Get-Date
            Write-Output "$Datum Lösche VMs aus der Liste" >> $env:APPDATA\..\Local\temp\log.txt
    $MgrArray = Get-VM -ComputerName $Hostname |
    Select-Object Name
            $Datum = Get-Date
            Write-Output "$Datum Lade alle Virtuellen Maschinen $($MgrArray)" >> $env:APPDATA\..\Local\temp\log.txt
            $Datum = Get-Date
            Write-Output "$Datum Fülle ComboBox1 mit VMNamen" >> $env:APPDATA\..\Local\temp\log.txt
    ForEach ($item in $MgrArray) {
        $ComboBox1.Items.Add($item.Name)}
    $Datum = Get-Date
    Write-Output "$Datum Fertig" >> $env:APPDATA\..\Local\temp\log.txt
    }

function Load-ComboBox-Hosts{
            $Datum = Get-Date
            Write-Output "[Lade Hosts]" >> $env:APPDATA\..\Local\temp\log.txt
            $Datum = Get-Date
            Write-Output "$Datum Lade Server mit dem Betriebsystem Windows Server in der Active Diretory" >> $env:APPDATA\..\Local\temp\log.txt
    $ComboBox4.Items.Clear()
            $Datum = Get-Date
            Write-Output "$Datum Lösche Host Liste" >> $env:APPDATA\..\Local\temp\log.txt
    $ComboBox4.Items.Add("localhost")
            $Datum = Get-Date
            Write-Output "$Datum Füge localhost zur Hostliste hinzu" >> $env:APPDATA\..\Local\temp\log.txt
    $HostArray = Get-ADComputer -Filter "OperatingSystem -like '*Server*'" | Select-Object Name
            $Datum = Get-Date
            Write-Output "$Datum Lade alle Hyper-V Hosts $($HostArray)" >> $env:APPDATA\..\Local\temp\log.txt
...
$RichTextBox1.Text = "Bitte einen Host auswählen"
            $Datum = Get-Date
            Write-Output "$Datum Gebe aus: Bitte einen Host auswählen" >> $env:APPDATA\..\Local\temp\log.txt


#Setzt die gewählte VM als VM und listet Infos auf
function Set-VM{
            $Datum = Get-Date
            Write-Output "[Set-VM]" >> $env:APPDATA\..\Local\temp\log.txt
    switch ($ComboBox1.Text){
        VM {[System.Windows.Forms.MessageBox]::Show("Bitte eine VM auswählen","MinersWin HyperV Manager",1)
            $Datum = Get-Date
            Write-Output "$Datum Keine VM Ausgewählt" >> $env:APPDATA\..\Local\temp\log.txt}
        default {
            $VMName = $ComboBox1.Text
            $Hostname = $ComboBox4.Text
            rm $env:APPDATA\..\Local\Temp\$($VMName).jpg
                    $Datum = Get-Date
                    Write-Output "$Datum Setze Host auf $($Hostname) und VM auf $($VMName)" >> $env:APPDATA\..\Local\temp\log.txt
            $VMInfo = Get-VM -ComputerName $Hostname -Name $VMName | select VMName, State, CPUUsage, MemoryAssigned, Uptime, MemoryStartup, ProcessorCount | Format-Table | Out-String
                    $Datum = Get-Date
                    Write-Output "$Datum Gib VMInfo aus `n $($VMInfo)" >> $env:APPDATA\..\Local\temp\log.txt
...
function Start-VM{
    $VMName = $ComboBox1.Text
    $Hostname = $ComboBox4.Text
    Start-VM -ComputerName $Hostname -Name $VMName -asJob
}
...
function Fill-Edit{
    Write-Output "[Fill-Edit]" >> $env:APPDATA\..\Local\temp\log.txt
        $Datum = Get-Date
        Write-Output "$Datum Fülle Textfelder aus" >> $env:APPDATA\..\Local\temp\log.txt
    $VMName = $ComboBox1.Text
    $Hostname = $ComboBox4.Text
    ...
    $TextBox12.Text = $VMName
        $Datum = Get-Date
        Write-Output "$Datum Lösche Textfelder und überprüfe Werte" >> $env:APPDATA\..\Local\temp\log.txt
    ...
    if ($Generation.Generation -eq "2"){
        ...
        $CheckBox5.Visible = $true
            $Datum = Get-Date
            Write-Output "$Datum Erzeuge SecureBoot Feld + Auswahl des Bootmediums, da es sich um eine Gen2 VM handelt" >> $env:APPDATA\..\Local\temp\log.txt
 ...
 #       if ($Firmware.BootOrder.First -eq "*EFI Network*"){
  #          $FirstBoot = "Netzwerk"
   #     } elseif ($Firmware.BootOrder -eq "Drive")p
        
    } else {
                $Datum = Get-Date
                Write-Output "$Datum Verstecke SecureBoot Feld + Auswahl des Bootmediums, da es sich um eine Gen1 VM handelt" >> $env:APPDATA\..\Local\temp\log.txt
...
function Get-Thumbnail{
            $Datum = Get-Date
            Write-Output "[Get-Thumbnail]" >> $env:APPDATA\..\Local\temp\log.txt
    ...
    $HyperVGuest = $VMName
    $ImagePath = "$env:APPDATA\..\Local\Temp"
    $xRes = 640
    $yRes = 480
            $Datum = Get-Date
            Write-Output "$Datum Erzeuge ein Screenshot der gewählten VM und speicher ihn ab" >> $env:APPDATA\..\Local\temp\log.txt
    ...
    $VMThumbnail.UnlockBits($VMThumbnailBitmapData);
            ...
            $Datum = Get-Date
            Write-Output "$Datum Setze den Screenshot als Bild in PictureBox1" >> $env:APPDATA\..\Local\temp\log.txt
    $VMThumbnail
    ...
    $Label38.Text = "$ImagePath\$HyperVGuest.jpg"
        $Datum = Get-Date
        Write-Output "$Datum Setze Dateipfad als Label" >> $env:APPDATA\..\Local\temp\log.txt
        $Datum = Get-Date
        Write-Output "$Datum Fertig" >> $env:APPDATA\..\Local\temp\log.txt
...
function Connect-VM{
            Write-Output "[Connect-VM]" >> $env:APPDATA\..\Local\temp\log.txt
    $VMName = $ComboBox1.Text
    $Hostname = $ComboBox4.Text
            $Datum = Get-Date
            Write-Output "$Datum Stelle Verbindung mit VM her und öffne VMConnect.exe" >> $env:APPDATA\..\Local\temp\log.txt
...
function Open-Manager{
        Write-Output "[Open-Manager]" >> $env:APPDATA\..\Local\temp\log.txt
            $Datum = Get-Date
            Write-Output "$Datum Öffne den Hyper-V Manager" >> $env:APPDATA\..\Local\temp\log.txt
    virtmgmt.msc
            $Datum = Get-Date
            Write-Output "$Datum Fertig" >> $env:APPDATA\..\Local\temp\log.txt    
}
...
function Save-Log{
    $File=Save-File
    cp $env:APPDATA\..\Local\temp\log.txt $File
}
...
function Send-Information{
    ...
    $datei = Get-content $env:APPDATA\..\Local\temp\log.txt | Out-String
